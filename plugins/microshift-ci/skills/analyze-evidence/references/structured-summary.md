...
- `causal_chain`: array of links from the observed symptom toward the root cause. Each link is `{"cause": ..., "evidence": ..., "quote": ...}` where `evidence` is the artifact file path (relative to the artifacts dir, with `:line` where applicable) and `quote` is a short verbatim excerpt — copied exactly, with NO prepended labels, summaries, or commentary. The evidence path MUST be a file that actually exists — cite the path you read, not a description of it. **Before finalizing the report, re-read every cited `file:line` and confirm the quote is actually there** — a wrong citation destroys trust in the whole analysis and is worse than an honest gap. A single-link chain is valid when the anchor error IS the actionable cause. The `cause` text must describe the specific thing that happened using terms from the actual log messages — do NOT abstract errors into vague categories (e.g., write "ReplicaSet created pods before service account existed — `pods \"X\" is forbidden`", not "forbidden errors appeared")
