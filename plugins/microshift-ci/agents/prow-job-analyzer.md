...
Read `plugins/microshift-ci/agents/references/microshift-ci-primer.md` at the start for artifact layout context (scenario naming, journal patterns, sosreport layout, timeout cascades, CI structure terms).

## SOS Report

**Journals:** use the plain-text `journal_*.log` files next to the sosreport tarballs — readable directly with Read/Grep. Check these FIRST.

**Pod logs** (`previous.log` — the only record of WHY a dead container exited) exist exclusively inside the sosreport tarball. Extract when the journal shows `CrashLoopBackOff`, `Back-off restarting`, repeated `Created container` events, or probe failures:
...
The script prints the extraction directory. For the extracted tree layout and on-failure vs end-of-scenario sosreport selection, see the primer. **Prefer the on-failure sosreport** — it contains the test's namespaces, which are cleaned up by the end-of-scenario report. Match sosreport to failure by capture time.
...
1. **Localize — identify the failed step and the anchor error**:
   - Scan the top level `build-log.txt` for the last `Running step ...` line before the container logs — this identifies the failed step. Then open that step's own `build-log.txt`. Fatal errors are often buried by teardown output — do not assume the error is at the end.
...
2. **Characterize — establish exactly WHAT failed before asking why**:
   ...
   - For each failing scenario, check the plain-text `journal_*.log` files for fatal patterns (panics, OOM kills, `leader election lost`, container exits). If the journal shows crashes or restarts, extract the sosreport (see SOS Report section) and read `previous.log`.
...
3. **Drill down — iterate hypothesis → evidence until the cause is actionable**:
   ...
   - Seek confirming or refuting evidence ONE LAYER DEEPER than the current log:
     - **Journal** — ALWAYS check the plain-text `journal_*.log` files for the scenario. Correlate with the failure timestamp (±5 minutes) and scan for OOM kills, segfaults, service restarts, disk pressure.
     - **Sosreport** — if the journal shows crashes or restarts, extract the sosreport (see SOS Report section) and read `previous.log`.
